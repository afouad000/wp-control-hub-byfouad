...
import { Globe, ShoppingCart, Package, Users as UsersIcon, Plus, AlertCircle, CheckCircle2 } from "lucide-react";
...
function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">Connected sites</CardTitle></CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <EmptyState
              ...
              action={<Button asChild size="sm"><Link to="/websites/new"><Plus className="mr-2 h-4 w-4" /> Add website</Link></Button>}
            />
          ) : (
            <div className="divide-y">
              {sites.map((s) => {
                const m = (s.meta ?? {}) as { woocommerce?: boolean };
                return (
                  <Link key={s.id} to="/websites/$id" params={{ id: s.id }} className="flex items-center justify-between gap-3 py-3 hover:bg-muted/30">
                    <div className="flex min-w-0 items-center gap-3">
                      <div className="grid h-9 w-9 shrink-0 place-items-center rounded-md border bg-muted/40"><Globe className="h-4 w-4" /></div>
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      ...
                      {s.status === "connected" ? (
                        <Badge variant="outline" className="border-success/40 text-success"><CheckCircle2 className="mr-1 h-3 w-3" /> Connected</Badge>
                      ) : (
                        <Badge variant="outline" className="border-destructive/40 text-destructive"><AlertCircle className="mr-1 h-3 w-3" /> {s.status}</Badge>
                      )}
                    </div>
                  </Link>
                );
